...
public class LabelResource {
     ...
     */
    @GetMapping("/labels")
    public ResponseEntity<List<LabelDTO>> getAllLabels(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("keyword-is-null".equals(filter)) {
            log.debug("REST request to get all Labels where keyword is null");
            return new ResponseEntity<>(labelService.findAllWhereKeywordIsNull(),
                    HttpStatus.OK);
        }
        if ("level-is-null".equals(filter)) {
            log.debug("REST request to get all Labels where level is null");
            return new ResponseEntity<>(labelService.findAllWhereLevelIsNull(),
                    HttpStatus.OK);
        }
