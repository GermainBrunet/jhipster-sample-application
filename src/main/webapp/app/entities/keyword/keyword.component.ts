...
export class KeywordComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.keywordService
      ...
      .subscribe(
        (res: HttpResponse<IKeyword[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  registerChangeInKeywords(): void {
    this.eventSubscriber = this.eventManager.subscribe('keywordListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IKeyword[] | null, headers: HttpHeaders, page: number): void {
